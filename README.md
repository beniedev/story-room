...
生成时，应用会把**本轮上下文包含的写作材料**发送给你配置的模型服务。本地优先描述的是书稿保存方式，不意味着真实 AI 请求始终留在设备内。密钥保存在单独的本地明文配置文件中，不放进书目备份。
...
保存和删除的事务恢复属于单一服务进程，不保证抵御断电、磁盘损坏，也不协调多个服务进程共享书库。当前没有账号同步、多人协作或桌面安装包。CI 覆盖 Windows、macOS、Ubuntu 与 Node 22/24 的测试和构建，不代表每一种浏览器或模型服务都已实测。
