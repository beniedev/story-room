...
When changing material loading, preserve these facts: the plot outline is future guidance; for `continue-section`, request `authorNote` takes precedence over `section.note`; other body-generation requests use the current `section.note`. Send a nonempty note verbatim as assistant history, then append a short user request to generate prose: `system → user context → assistant note → user generation request`. This is history guidance, not trailing prefill. Empty notes and summary requests retain `system → user`. Token estimates include the closing request; Providers preserve message order without changing roles and retrying after errors. The app does not write the note into manuscript blocks or append it to generated results; Section Memory must be fresh and confirmed; and an unconfirmed previous-text selection must not be written to the Book. Both author and character modes must go through the same persistence/context/Provider/apply pipeline.
