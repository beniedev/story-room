...
| Host entry | The local host accepts loopback requests from the current machine | LAN/public network exposure, network observers, and other users on the network |
...
- The host and development launcher accept loopback addresses only. Non-loopback binding is unsupported. State-changing requests with an Origin header require same-origin.
...
- The local host uses HTTP and is not a hardened public service. Loopback binding does not provide TLS, user accounts, rate limiting, or multi-user authorization.
