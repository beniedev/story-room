...
The existing controls and steps stay the same. For **Send and continue writing**, **Generate answer**, and **Regenerate version** in either mode, Story Room sends the note's original text as assistant history, followed by a short user request to generate prose for the preceding writing task. This supports services that require a final user turn; it is history guidance, not trailing assistant prefill. Clear the note to remove it from future requests. The app does not save the note as a manuscript block or append it to the returned text. JSON full backups retain it; EPUB, Markdown, and TXT reading exports omit it. Story Room does not change the note's role and retry after an error or guarantee that the service will follow it.
