...
The local host defaults to `127.0.0.1` and accepts an explicitly configured LAN or private-network bind address. It does not add an access-password, token, or Host allowlist. State-changing browser requests with an `Origin` header must be same-origin. Configured HTTP/HTTPS Providers on public and private networks require no extra opt-in flag; metadata, link-local, and redirect targets remain blocked.
...
- Book saves have no application-defined request size cap, and autosave currently writes the whole Book. Incremental Section/source revisions are future work, not part of this ADR's implementation.
