...
- Regenerating an answer uses adopted manuscript text before its target block; the target answer, alternative candidates, and later passages are excluded. Responding to a final user block includes that user's text once. Candidate browsing does not change the adopted manuscript; adoption updates its content projection and the existing Memory freshness checks.
