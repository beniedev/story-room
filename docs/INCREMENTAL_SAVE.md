...
This is a design note, not an implemented API. The current host accepts a whole-Book `PUT`, limits the request to 1 MB, and autosaves the whole Book. That keeps the first demo simple but makes large Books expensive to write and easy to lose between revisions.
...
5. Keep the request bounded per revision and keep exports as explicit whole-Book operations.
