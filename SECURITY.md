...
Device Book, library-index, and recovery-draft writes require a page-lifetime exclusive Web Lock named `story-native:device-library-writer:v1`. The page determines its role before initializing the library or restoring a draft, and storage mutation helpers also check the active page's writer lease. Other tabs read persisted Books without repairing storage or reading writer drafts. They can export saved content and explicitly request editing after the previous writer closes; takeover reloads persisted state before draft-conflict recovery and editing. Requests use `ifAvailable` without background retries, lock stealing, or a storage-based fallback.

Device editing requires the [Web Locks API](https://www.w3.org/TR/web-locks/) in a secure context. Unsupported browsers, insecure contexts, and lock failures leave the page read-only with saved-Book reading and export available. Local-host APIs and filesystem transactions do not use this device lock.
